...
//    }()
    ...
    @objc private func repsSliderChanged() {
        ...
        setInactive(label: timerLabel, numberLabel: numberOfTimerLabel, slider: timerSlider)
    ...
    @objc private func timerSliderChanged() {
        ...
        // если секунды не равны 0, то выводим минуты и секунды, иначе только минуты
        numberOfTimerLabel.text = (sec != 0 ? "\(min) min \(sec) sec" : "\(min) min")

        setInactive(label: repsLabel, numberLabel: numberOfRepsLabel, slider: repsSlider)
        setActive(label: timerLabel, numberLabel: numberOfTimerLabel, slider: timerSlider)
    }
    // меняем прозрачность у активного элемената
    private func setActive(label: UILabel, numberLabel: UILabel, slider: UISlider) {
        label.alpha = 1
        numberLabel.alpha = 1
        slider.alpha = 1
    }
    // меняем прозрачность у неактивного элемената, обнуляем значение
    private func setInactive(label: UILabel, numberLabel: UILabel, slider: UISlider) {
        label.alpha = 0.5
